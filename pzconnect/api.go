...
func Broadcast(payload []byte) []uint64 {
	...
	tracelog.Trace("pzconnect", "Broadcast", "failedIDs %v", failedIDs)
	return failedIDs
}
...
func SendMessage(receiverIDs []uint64, payload []byte) []uint64 {
	...
	for _, receiverID := range receiverIDs {
		success := sendMessage(receiverID, payload)
		if !success {
			failedIDs = append(failedIDs, receiverID)
		}
	}

	tracelog.Trace("pzconnect", "SendMessage", "failedIDs %v", failedIDs)
	return failedIDs
}

func sendMessage(receiverID uint64, payload []byte) bool {
	tracelog.Trace("pzconnect", "sendMessage", "Target Client Id: %v", receiverID)
	conn := clientsHub.Get(receiverID)
	if conn == nil {
		tracelog.Trace("pzconnect", "sendMessage", "receiver: %v is not available", receiverID)
		return false
	}
	...
	buf, err := createBuf(messagePack, payload)
	if err != nil {
...
func createBuf(messagePack []byte, payload []byte) ([]byte, error) {
